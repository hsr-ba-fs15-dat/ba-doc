...
\xxx[Architektur-doc finden/erstellen (evtl selbstgemachte grafik mit tiers/zusammenspiel apis (levelup-leveldown/sqldown-leveldb/sql))]
...
\section{Gasket}\label{sec:gasket} % https://github.com/datproject/gasket
...
Ein Beispiel einer trivialen \path{package.json} Konfiguration wird in \cref{lst:gasket-beispiel} aufgezeigt und gibt lediglich ``HELLO WORLD'' aus .

\begin{srclst}[label=lst:gasket-beispiel]{json}{Offizielles Gasket Beispiel}
{
  "name": "my-test-app",
  "dependencies" : {
...
\paragraph{Antwort GET /api} Liefert allgemeine Informationen zur dat-Instanz
...
Das Kommandozeilen-Tool \texttt{dat} kann zur Erstellung oder für rudimentäre Inspektion einer dat-Instanz verwendet werden. Für fortgeschrittenere Benutzung ist es jedoch nicht geeignet, da viele Optionen fehlen. Stattdessen sollte das JavaScript-API verwendet werden.

Zudem ermöglicht es, entfernte \gls{dat} Repositories analog git zu ``klonen'' und Änderungen ebenfalls analog git wieder zu ``pushen''. Spätere Versionen sollen auch Branches erlauben sowie Konflikte behandeln können.
