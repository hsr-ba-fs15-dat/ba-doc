...
\mytable{lX}{
	...
	\textbf{Schema/Format Experte} & Ist oftmals ein Datenanbieter selbst bzw. eine Person innerhalb dessen Organisation. Kann Schema-Transformationen mit OpenDataHub durchführen und ist dafür zuständig dem Administrator von OpenDataHub allfällige Module zu liefern zur Unterstützung deren Format(e) innerhalb der Applikation.\\
	...
	\textbf{Administrator} & Der Applikationsverantwortliche, welche die Applikation betreibt und wartet.\\
...
\begin{scrumepic}[label=epic:pd:publizieren]{Daten publizieren}
	...
	\storyacceptance	
	Gegeben
	\storyitem{einer oder mehreren Dateien in einem von der Applikation unterstützten Formaten}
...
\begin{scrumstory}[label=story:pd:online-quelle]{Online Quellen}
	Als Datenanbieter, will ich Daten in einem strukturierten Format mit Angabe einer Online Quelle (HTTP, FTP, WFS) publizieren können, sodass ich diese auf der Plattform weiterverwenden kann.
	\storyacceptance	
	Gegeben
...
\begin{scrumstory}[label=story:pd:private-daten]{Private Daten}
	...
	wenn ich den Eintrag als Privat markieren, dann sollen die Daten für anonyme oder andere Benutzer nicht (mehr) in der Liste von verfügbaren Daten gelistet sein und jeglicher Zugriff darauf verweigert werden.
\end{scrumstory}

\begin{scrumstory}[label=story:pd:parser]{Modul anbieten}
	Als Datenanbieter bzw. Schema/Format Experte, will ich Python-Module (Plugins) die eine bestimmte Schnittstelle implementieren, anbieten können, um die bereitgestellten Daten/Formate unterstützen zu können.
	\storyacceptance	
	Gegeben
...
\begin{scrumepic}[label=epic:pd:transformation]{Transformation}
	...
	wenn eine solche Transformation an die Applikation übergebe, dann soll diese auf die ausgewählten Daten angewendet werden und das Resultat analog wie einzelne Daten in \cref{epic:pd:datenbezug} in diversen Formaten beziehen können.
\end{scrumepic}

\begin{scrumstory}[label=story:pd:views]{Transformation publizieren}
	Als Experte will ich diese Transformationen (``Views'' auf die Daten) analog wie die einzelnen Daten, speichern und publizieren können, sodass diese einerseits immer wieder per Knopfdruck angewendet werden können und andererseits auch von anderen, nicht-Experten verwendet werden können.
\end{scrumstory}
...
Die Anforderungen an die eingesetzten Technologien\footnote{Programmiersprachen, Frameworks, Libraries, Services, \dots} ist teilweise durch die Aufgabenstellung sowie durch die bereits existierende Infrastruktur am Geometa Lab der HSR wie folgt eingeschränkt:
