...
Diese Arbeit befasst sich nur mit tabellarischen Daten. 
...
\mytable{lX}{
  ...
  \textbf{String-Operationen} & Folgende Operationen sollen unterstützt werden: find, substring, replace, trim \\
  \textbf{Geo-Operationen} & \\
...
\xxx[geo-ops: was für operationen sind üblich?]

\xxx[Ausformulieren. Can't find the words right now.]
\subsection{Vereinfachtes SQL}
Dieses Format lehnt sich an SQL an, ohne für die Transformation unnötige Features zu unterstützen.

Vorteil: Familiarität für Personen mit Datenbank-Erfahrung
Nachteil: SQL ist zwar standardisiert, das bedeutet jedoch nicht, dass nicht grosse Unterschiede zwischen verschiedenen Anbietern bestünden. Dies wird durch Einschränkung auf ein vereinfachtes Format gemildert.

\subsection{ogrtools: ogrtransform}
Erweiterung und Verwendung des ogrtransform-Formats. Dies limitiert die Transformation auf von OGR/GDAL unterstützte Daten-Arten.

Vorteil: Bereits existierende Basis.
Nachteil: ogrtransform ist momentan limitiert auf das Mappen von Feldern sowie das hinzufügen von neuen Feldern mit Default-Werten.

\subsection{Eigenes Format}
Eigenes Format.

Vorteil: Komplette Freiheit
Nachteil: Eigener Parser (vereinfacht durch pyparsing)
