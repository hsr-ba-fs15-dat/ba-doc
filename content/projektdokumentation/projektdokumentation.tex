...
}{Bewertungs-Kriterien mit Gewichtung}{pd:transformation-lang-criteria}

\cref{tab:pd:transformation-lang-results} fasst die verschiedenen Optionen zusammen.
\mytable{lXXXX}{
  & \textbf{Datenbank: SQL} & \textbf{ogrtransform} & \textbf{DSL: SQL} & \textbf{DSL: Eigenes Format} \\
  \midrule
...
Die Syntax von OdhQL orientiert sich stark an ANSI SQL.
...
}{Durch OdhQL implementierte Features}{pd:transformation-lang-impl}
...
%\begin{description}
%\item[Feld] Name des Feldes, mit dem Namen oder Alias der Datenquelle als Prefix. Feld-Namen und Aliases können in doppelten Anführungszeichen geschrieben werden, z.B. wenn Sonder- oder Leerzeichen verwendet werden sollen. \\
%Beispiel: \texttt{a.beschreibung}, \texttt{a.geom as geometry}, \texttt{a.``Zentroid X [m]''}
%\item[Wert] Integer, Float oder String (in einfachen Anführungszeichen). Es muss zwingend ein Alias angegeben werden. \\
%Beispiel: \texttt{1 as one}, \texttt{3.14 as pi}, \texttt{`moon' as cheesefactory} \xxx[fix joke. it's broken.]
...
%Beispiel: \texttt{cast(a.pi, `float') as pi}, \\
%          \texttt{concat('POINT(', cast(v0.x, `string'), ` ', cast(v0.y, `string'), `)')) as geometry}
%\end{description}
%
%Die Liste der Datenquellen besteht aus einer Datenquelle sowie optional einer Liste von Join-Definitionen. 
